...
# TODO: Check if this np.array is redundant. Not sure if the input is actually a bool array. 
def has_noncovered_zero(m,cov_row, cov_col):
    return np.any((m == 0) & np.invert(np.array(cov_row, dtype=bool)) & np.invert(np.array(cov_col, dtype=bool)))
